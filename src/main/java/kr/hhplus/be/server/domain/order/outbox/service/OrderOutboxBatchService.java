...
public class OrderOutboxBatchService {
    ...
    private final String ORDER_COMPLETED = "order-completed";
    ...
    public void retryFailedOrderCompletedMessages() {
        ...
        for (OrderOutbox outbox : failedMessages) {
            try {
                OrderKafkaProducer.send(ORDER_COMPLETED, outbox.getMessageId(), outbox.getPayload());
                outbox.updateStatus(OrderOutboxStatus.SUCCESS);
